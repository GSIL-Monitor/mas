...
import com.letv.mas.router.omp.util.IdGenerator;
...
public class SSOLoginService {
    ...
    public String checkLogin(String jsoncallback, String user, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isNotBlank(loginUser)) {
            ...
            if (userByMail == null) {
                long userId = new IdGenerator().nextId();
                ssoLoginMapper.insertUser(loginUser, userId + "");
            ...
            List showAcl = callbackService.redisUtil().lGet(userByMail.getUser_id() + "", 0, -1);
            if (showAcl != null && showAcl.size() == 0 && allAcl != null && allAcl.size() != 0) {
                ...
                callbackService.redisUtil().lSet(userByMail.getUser_id() + "", showAcl);
                callbackService.redisUtil().expire(userByMail.getUser_id() + "", 1800);
            }
            ...
            if (StringUtils.isNotBlank(loginUser)) {
                ...
                try {
                    ...
                    byte[] bytes = userByMail.getUser_id().getBytes("UTF-8");
                    userId = encoder.encodeToString(bytes);
                    //解码
//                    System.out.println(new String(decoder.decode(str), "UTF-8"));
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
                map.put("OMP_LOGIN_TOKEN", userId);
                map.put("OMP_LOGIN_USER", str);
                return jsoncallback + "(" + new Gson().toJson(map) + ")";
            }
        }
